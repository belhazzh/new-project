...
class CartListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            try:
                context["objects"] = Commande.objects.get(client=self.request.user.client).items.all()
                nb_item = Commande.objects.get(client=self.request.user.client).items.all().count()
                for item in Commande.objects.get(client=self.request.user.client).items.all():
